...
scene_wrapped_lambert :: proc(
    ...
    if !app.scene_direction_valid(normal) ||
       !app.scene_direction_valid(surface_to_light) {
        return 0
    }
...
scene_distance_attenuation :: proc(distance, light_range: f32) -> f32 {
    if !app.scene_f32_finite(distance) || !app.scene_f32_finite(light_range) ||
       distance < 0 || light_range < app.SCENE_MIN_LIGHT_RANGE {
        return 0
    }
...
scene_spot_attenuation :: proc(
    ...
    if !app.scene_f32_finite(theta) ||
       !app.scene_f32_finite(inner_angle_deg) ||
       !app.scene_f32_finite(outer_angle_deg) ||
...
scene_light_band_input :: proc(energy: rl.Vector3) -> f32 {
    if !app.scene_vector3_finite(energy) {
        return 0
    }
