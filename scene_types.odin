...
scene_wrapped_lambert :: proc(
    ...
    if !scene_direction_valid(normal) ||
       !scene_direction_valid(surface_to_light) {
        return 0
    }
...
scene_distance_attenuation :: proc(distance, light_range: f32) -> f32 {
    if !scene_f32_finite(distance) || !scene_f32_finite(light_range) ||
       distance < 0 || light_range < SCENE_MIN_LIGHT_RANGE {
        return 0
    }
...
scene_spot_attenuation :: proc(
    ...
    if !scene_f32_finite(theta) ||
       !scene_f32_finite(inner_angle_deg) ||
       !scene_f32_finite(outer_angle_deg) ||
    ...
    t := clamp((theta - outer_cos) / max(inner_cos - outer_cos, f32(0.0001)), f32(0), f32(1))
    return t * t * (3 - 2 * t)
}

scene_light_band_input :: proc(energy: rl.Vector3) -> f32 {
    if !scene_vector3_finite(energy) {
        return 0
    }
